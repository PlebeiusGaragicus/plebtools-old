...
from BitcoinData import ONE_HUNDRED_MILLION, block_subsity
...
def update_break_even(callback_throwaway):
    ...
    be_nh = (reward * hashrate * (1 - poolfee) * price_satoshi * 6000) / (
        rate * wattage
    )
    be_p = ONE_HUNDRED_MILLION * (
        (nh * rate * wattage) / (reward * hashrate * (1 - poolfee) * 6000)
    )
    be_rate = (reward * hashrate * (1 - poolfee) * price_satoshi * 6000) / (
        nh * wattage
    )
...
def main() -> None:
    ...
    try:
        nh = (
            int(ur.urlopen(ur.Request("https://blockchain.info/q/hashrate")).read())
            / 1000
        )
        height = int(
            ur.urlopen(ur.Request("https://blockchain.info/q/getblockcount")).read()
        )
        price = int(
            float(ur.urlopen(ur.Request("https://blockchain.info/q/24hrprice")).read())
        )
    except:
    ...
    with output.use_scope("main", clear=True):
        output.put_info(
            "Enter the details of a bitcoin miner and this will calculate the break-even price"
        )
        output.put_table(
            tdata=[
                [
                    ...
                    pin.put_input(
                        name="hashrate", type="float", label="Hashrate (terahash)"
                    ),
                    pin.put_input(
                        name="poolfee", type="float", label="Pool fee %", value=0
                    ),
                ]
            ]
        )
        ...
        output.put_table(
            tdata=[
                [
                    pin.put_input(
                        name="height", type="float", label="block height", value=height
                    ),
                    pin.put_input(
                        name="subsidy",
                        type="text",
                        label="current block subsidy",
                        value=f"{block_subsity(height):,}",
                        readonly=True,
                    ),
                    pin.put_input(
                        name="blocktxfee",
                        type="float",
                        label="Average block fees",
                        value=9_000_000,
        ...
        output.put_table(
            tdata=[
                [
                    pin.put_input(
                        name="rate", type="float", label="Cost / kWh", value=0.12
                    ),
                    pin.put_input(
                        name="nh",
                        type="float",
                        label="Network hashrate (terahash)",
                        value=nh,
                    ),
                    pin.put_input(
                        name="price", type="float", label="Bitcoin price", value=price
                    ),
                ]
            ]
        )
        ...
        output.put_table(
            tdata=[
                [
                    pin.put_input(
                        name="be_rate", type="text", label="Cost / kWh", readonly=True
                    ),
                    pin.put_input(
                        name="be_nh",
                        type="text",
